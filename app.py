...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///quantum_vault.db')
...
print(f"Access Key ID: {app.config['AWS_ACCESS_KEY_ID']}")
print(f"Bucket Name: {app.config['AWS_BUCKET_NAME']}")
...
# S3 client setup
def get_s3_client():
    print("\nCreating S3 client with:")
    print(f"Access Key ID: {app.config['AWS_ACCESS_KEY_ID']}")
    print(f"Region: {app.config['AWS_REGION']}")
    print(f"Bucket: {app.config['AWS_BUCKET_NAME']}")
...
def store_password_in_s3(encrypted_password, user_id, entry_id):
    ...
    try:
        ...
        print(f"Error details: {str(e.response)}")
        print(f"Using region: {app.config['AWS_REGION']}")
        print(f"Using bucket: {app.config['AWS_BUCKET_NAME']}")
        raise
